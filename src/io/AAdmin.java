...
public class AAdmin {
	...
	static final Logger logger = LogManager.getLogger("io.AAdmin");
				...
				try {
					...
					case 1:
						...
						if (Files.exists(dir) && Files.isDirectory(dir)) {
							...
							paths = NIO2RecursiveDir.walkDir(dir, paths);
						...
						if (guardar == 1) {
							...
							// Escribe tu codigo aqui {
							for(Path path : paths) {
								lineas.add(path.toString());
							}

							// }

							/*
							 * Resultado lo tiene que guardar dentro de un archivo con el formato
							 * directorio.txt
							 */
							Path guardarDir = Paths.get(dir.getFileName() + ".txt");
							/*
							 * 5. Escribe el archivo en guardarDir con el contenido de la variable lineas,
							 * Character set UTF_8 y opcion CREATE
							 */
							// Escribe tu codigo aqui {
							BufferedWriter writer = new BufferedWriter(new FileWriter(guardarDir.toString()));
